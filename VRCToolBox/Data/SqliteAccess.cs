...
namespace VRCToolBox.Data
{
    internal static class SqliteAccess
    {
        internal static async Task InitializeAsync()
        {
            ...
            tasks.Add(InitializeLogDBAsync());
            tasks.Add(InitializePhotoDBAsync());
        ...
        internal static void InsertUserActivity(List<string[]> parametersList)
        {
            ...
            using (SqliteConnection connection = new SqliteConnection($@"Data Source={ProgramSettings.Settings.UserActivityDBPath}\{ProgramConst.VRChatLogDBName}{ProgramConst.FileExtensionSQLite3}"))
            {
                ...
                using(SqliteCommand command = connection.CreateCommand())
                {
                    ...
                    commandText = $@"INSERT INTO USER_ACTIVITY (ACTIVITY_TIME, ACTIVITY_TYPE, WORLD_ID, USER_NAME, FILE_NAME) VALUES (:ACTIVITY_TIME, :ACTIVITY_TYPE, :WORLD_ID, :USER_NAME, :FILE_NAME);";
                    command.CommandText = commandText;
                    ...
                    foreach (string[] parameter in parametersList)
                    {
                        ...
                        command.Parameters.AddWithValue($@":WORLD_ID", parameter[2]);
        ...
        internal static void InsertWorldVisit(List<string> parametersList)
        {
            ...
            using (SqliteConnection connection = new SqliteConnection($@"Data Source={ProgramSettings.Settings.WorldDataDBPath}\{ProgramConst.VRChatLogDBName}{ProgramConst.FileExtensionSQLite3}"))
            {
                ...
                using (SqliteCommand command = connection.CreateCommand())
                {
                    ...
                    commandText = $@"INSERT INTO WORLD_VISIT (VISIT_TIME, WORLD_NAME, WORLD_ID, FILE_NAME) VALUES (:VISIT_TIME, :WORLD_NAME, :WORLD_ID, :FILE_NAME);";
                    command.CommandText = commandText;

                    command.Parameters.Clear();
                    command.Parameters.AddWithValue(@$":WORLD_ID"  , parametersList[0]);
                    command.Parameters.AddWithValue(@$":VISIT_TIME", parametersList[1]);
                    command.Parameters.AddWithValue(@$":WORLD_NAME", parametersList[2]);
                    command.Parameters.AddWithValue(@$":FILE_NAME" , parametersList[3]);
                    command.ExecuteNonQuery();
